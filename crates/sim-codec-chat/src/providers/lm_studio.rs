...
pub fn decode_lm_studio_request(input: Input) -> Result<Expr> {
    decode_lm_studio_request_with_limits(input, DecodeLimits::default())
}

/// Decodes an LM Studio OpenAI-compatible request body under explicit limits.
pub fn decode_lm_studio_request_with_limits(input: Input, limits: DecodeLimits) -> Result<Expr> {
    decode_request_for_provider(LM_STUDIO_CODEC_ID, input, PROVIDER, limits)
}
...
pub fn decode_lm_studio_response(
    ...
    decode_lm_studio_response_with_limits(runner, model, body, include_raw, DecodeLimits::default())
}

/// Decodes an LM Studio OpenAI-compatible response body under explicit limits.
pub fn decode_lm_studio_response_with_limits(
    runner: Symbol,
    model: &str,
    body: &[u8],
    include_raw: bool,
...
pub fn decode_lm_studio_stream(
    ...
    decode_lm_studio_stream_with_limits(runner, model, body, include_raw, DecodeLimits::default())
}

/// Decodes LM Studio OpenAI-compatible SSE chunks under explicit limits.
pub fn decode_lm_studio_stream_with_limits(
    runner: Symbol,
    model: &str,
    body: &[u8],
    include_raw: bool,
