...
//! trees and exposes `ParseCx`, `SpannedToken`, the `decode_algol_located`
//! family, and `algol_pratt_shape` for Shape-backed infix matching. `pratt`
//! holds the precedence-climbing parser (`PrattParser`) and the operator table
//! (`default_pratt_table`, `supports_pratt`). `encode` renders any `Expr` back
//! to infix text (`encode_algol`). `runtime` provides the `Lib` registration
//! (`AlgolCodec`, `AlgolCodecLib`) that wires the codec into the runtime.
#![forbid(unsafe_code)]
#![deny(missing_docs)]
...
pub use parse::{
    AlgolShapeParser, ParseCx, SpannedToken, algol_pratt_shape, decode_algol_located,
    decode_algol_located_with_budget, parse_algol_expr_with_table,
