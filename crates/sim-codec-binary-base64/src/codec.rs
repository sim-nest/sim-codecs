//! The `BinaryBase64Codec` runtime object and its `Lib` registration.
//!
//! Implements the codec traits by delegating frame encode/decode to
//! `sim-codec-binary` and adding the base64 text wrapping on either side.
...
impl Encoder for BinaryBase64Codec {
    fn encode(&self, _cx: &mut WriteCx<'_>, expr: &Expr) -> Result<Output> {
        let frame = sim_codec_binary::encode_frame(expr)?;
...
impl LocatedEncoder for BinaryBase64Codec {
    fn encode_located(&self, cx: &mut WriteCx<'_>, expr: &LocatedExpr) -> Result<Output> {
        let frame = sim_codec_binary::encode_located_frame(expr, cx.options.lossless_origin)?;
        Ok(Output::Text(encode_base64(&frame.0)))
    }
}
...
impl TreeEncoder for BinaryBase64Codec {
    fn encode_tree(&self, cx: &mut WriteCx<'_>, expr: &LocatedExprTree) -> Result<Output> {
        ...
        let frame = sim_codec_binary::encode_located_tree_frame(expr, cx.options.lossless_origin)?;
        Ok(Output::Text(encode_base64(&frame.0)))
    }
}
...
fn decode_tree(cx: &mut ReadCx<'_>, input: Input) -> Result<LocatedExprTree> {
    ...
    sim_codec_binary::decode_located_tree_frame_with_limits(
        cx.codec,
        &bytes,
        sim_codec_binary::DecodeLimits::from(cx.limits),
    )
    .map(|(_, tree)| tree)
}
...
fn input_text(codec: CodecId, input: Input) -> Result<String> {
    match input {
        ...
        Input::Bytes(bytes) => String::from_utf8(bytes).map_err(|err| Error::CodecError {
            codec,
            message: format!("binary-base64 input is not valid UTF-8: {err}"),
        }),
    }
}
...
/// all codec roles.
pub struct BinaryBase64CodecLib {
    symbol: Symbol,
    codec_id: CodecId,
}

impl BinaryBase64CodecLib {
    /// Creates the codec lib bound to the runtime-assigned `id` for
    /// `codec/binary-base64`.
    pub fn new(id: CodecId) -> Self {
        Self {
            symbol: Symbol::qualified("codec", "binary-base64"),
            codec_id: id,
        }
    }
}

impl Lib for BinaryBase64CodecLib {
    ...
    fn load(&self, _cx: &mut sim_kernel::LoadCx, linker: &mut Linker) -> Result<()> {
        ...
        linker.codec_value(
            ...
            codec_value(CodecRuntime {
                id: self.codec_id,
                symbol: self.symbol.clone(),
                decoder: Some(Arc::new(BinaryBase64Codec)),
                located_decoder: Some(Arc::new(BinaryBase64Codec)),
                tree_decoder: Some(Arc::new(BinaryBase64Codec)),
                encoder: Some(Arc::new(BinaryBase64Codec)),
                located_encoder: Some(Arc::new(BinaryBase64Codec)),
                tree_encoder: Some(Arc::new(BinaryBase64Codec)),
                expr_shape,
                options_shape,
